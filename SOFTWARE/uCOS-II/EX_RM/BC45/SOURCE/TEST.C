...
void PeriodicTask(void *pdata)
{
    ...
    for (;;)
    {
        ...
        sprintf(s, "Task%d : start=%5lu  end=----  period=%5lu  run=%4lu",
        ...
        end_tick = start_tick + OSTCBCur->OSTCBExecTime;
        end_sec  = (end_tick - MyStartTime) / OS_TICKS_PER_SEC;

        sprintf(s, "Task%d : start=%5lu  end=%4lu  period=%5lu  run=%4lu",
                display_id, (unsigned long)start_sec, (unsigned long)end_sec,
                (unsigned long)period_sec, (unsigned long)run_count);
        PC_DispStr(0, row, s, DISP_FGND_BLACK + DISP_BGND_LIGHT_GRAY);
