...
4. 用 `PC_DispStr` 輸出此 task 的執行資訊（row 用 13+task_id），**顯示時除以 `OS_TICKS_PER_SEC` 換算為秒**：
   ```c
   sprintf(s, "[t=%4lds] Task%d  exec=%lds period=%lds",
           OSTimeGet()/OS_TICKS_PER_SEC, task_id,
