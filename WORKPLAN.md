...
每次迴圈代表一個執行週期，row 用 `11 + task_id`：

1. 記錄開始時間（`OSTimeGet()`），遞增執行計數
2. **忙碌等待前**：用 `PC_DispStr` 輸出 context switch 時刻（start 時間、period、計數），end 欄位先填 `----s`
3. 忙碌等待直到經過 `OSTCBCur->OSTCBExecTime` ticks（期間高優先權 task 可搶占）
4. 記錄結束時間，計算 elapsed
5. **忙碌等待後**：覆蓋同一 row，填入實際 end 時間
6. 計算剩餘等待時間 `OSTCBPeriod - elapsed`，用 `OSTimeDly` 等到下一個週期

顯示時間時除以 `OS_TICKS_PER_SEC` 換算為秒。`task_id` 從 pdata 取得。
